...
export default function RentHistory() {
    ...
    const deleteRequest = (orderId) => {
        db.collection("requestedItems").doc(orderId).delete()
            .then(() => {
                toast.success("Requested Deleted Successfully!!");
                setOrders(prevOrders => prevOrders.filter(order => order.orderId !== orderId));
            })
            .catch((error) => {
                toast.error("Error Deleting Request. Please try after sometime!!");
            ...
            if (user) {
                ...
                const unsubscribe = userRef.onSnapshot(async (doc) => {
                    if (doc.exists) {
                        ...
                        const ordersID = doc.data().myRequestedItems || [];
                        const promises = ordersID.map((orderID) => {
                            const orderRef = db.collection('requestedItems').doc(orderID);
                                ...
                                <div className="modal-body text-start text-black p-4">
                                    ...
                                    <div className="row">
                                        <div className="col mb-1">
                                            <p className="small text-muted mb-1">Renter</p>
                                            <p>{order.name}</p>
                                        </div>
                                        <div className="col mb-1">
                        ...
                        <div className="osahan-account-page-right shadow-sm bg-white p-4 h-100">
                            <div className="tab-content" id="myTabContent">
                                <div className="tab-pane fade show active" id="orders" role="tabpanel" aria-labelledby="orders-tab">
                                    <h2 className="font-weight-bold mt-0 mb-4">My Requested ADS</h2>
                                    {!isLoading && orders.length === 0 && <NoRent />}
                                                        ...
                                                        <div className="p-4">
                                                            ...
                                                            <div className="d-flex flex-column align-items-start mb-3">
                                                                ...
                                                                <div className="d-flex align-items-center mb-1">
                                                                    <i className="icofont-location-arrow me-2 text-secondary">Renter: {order.name}</i>
                                                                </div>
                                                                <div className="d-flex align-items-center mb-1">
                                                            ...
                                                            <div className="d-flex justify-content-between">
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={() => deleteRequest(order.orderId)}
                                                                >
                                                                    <i className="icofont-headphone-alt me-2"></i> DELETE REQUEST
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
